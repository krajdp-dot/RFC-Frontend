  ...
  return (
    <div className="bg-card border rounded-xl overflow-hidden flex flex-col h-full">
      <div className="p-4 border-b flex justify-between items-center">
        ...
        <Link href="/payables" className="text-sm p-0 h-auto font-medium flex items-center gap-1 text-primary hover:underline">View All <ArrowRight className="w-3 h-3" /></Link>
      </div>
      
      <div className="p-5 flex flex-col grow justify-between">
        <div className="mb-4">
          <div className="text-2xl font-bold tracking-tight tabular-nums">{formatCurrency(payables.total)}</div>
        ...
        <div className="flex items-center justify-between text-sm font-medium mt-auto border-t pt-5">
          <div className="flex flex-col">
            ...
            <span className="text-red-500 tabular-nums">{formatCompact(payables.overdue)}</span>
          </div>
          <div className="flex flex-col">
            <span className="text-xs text-muted-foreground font-normal mb-1">Due today</span>
            <span className="text-amber-500 tabular-nums">{formatCompact(payables.dueToday)}</span>
          </div>
          <div className="flex flex-col">
            <span className="text-xs text-muted-foreground font-normal mb-1">Due this week</span>
            <span className="text-foreground tabular-nums">{formatCompact(payables.dueThisWeek)}</span>
          </div>
        </div>
      </div>
    </div>
